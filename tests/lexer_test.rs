...
fn test_indent_calculation() {
    ...
    while let Some(token) = lexer.next_token() {
        println!(
            "Token: kind={}, value={:?}, row={}, col={}, indent={}",
    ...
    assert_eq!(
        identifiers[1].indent, 2,
        "Second token should have 2 spaces indent"
    );

    // Third token: 4 spaces indent
    assert_eq!(identifiers[2].value, "four_spaces");
    assert_eq!(
        identifiers[2].indent, 4,
        "Third token should have 4 spaces indent"
    );
}
